...
export default async function checkLoginMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if(user.isBanned) throw new AppError('Sua conta foi banida, caso ache que isto é um erro, entre em contato com o administrador.', 403);
    ...
    // Salvando o ID do usuário na requisição
    req.user = {
        id: user.id,
        name: user.name,
        email: user.email,
